    ...
    <Layout>
      ...
      <section>
        ...
        <div style={{ padding: '1rem', margin: '1rem 0', borderLeft: '2px solid #eaeaea', visibility: `${showAnswer ? 'visible' : 'hidden'}`}}>
          ...
          <div style={{ marginBottom: '0.25rem'}}>
            <span>onyomi : </span>
            {items[currentIndex].readings.onyomi.map((reading, index) => {
              return (
                <>
                  {index > 0 && ', '}
          ...
          <div style={{ marginBottom: '0.25rem'}}>
            <span>kunyomi : </span>
            {items[currentIndex].readings.kunyomi.map((reading, index) => {
              return (
                <>
                  {index > 0 && ', '}
                  <span key={index}>{reading}</span>
          ...
          <div style={{ marginBottom: '0.25rem'}}>
            ...
            {items[currentIndex].primitives && items[currentIndex].primitives.FR.map((primitive, index) => {
              return (
                <>
                  {index > 0 && ', '}
                  <span key={index}>{primitive}</span>
                </>
              ) 
            })}
          </div>
